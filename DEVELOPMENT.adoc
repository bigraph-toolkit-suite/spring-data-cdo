...
If you want to build with the regular `mvn` command, you will need https://maven.apache.org/download.cgi[Maven v3.8.7 or above].
...
----
# Deploy all modules
$ mvn clean deploy -DskipTests -P release,central -DskipNexusStagingDeployMojo=true

# Deploy single modules
$ mvn clean deploy -DskipTests -P release,central -pl :spring-data-cdo -DskipNexusStagingDeployMojo=true
$ mvn clean deploy -DskipTests -P release,central -pl :spring-data-cdo-distribution -DskipNexusStagingDeployMojo=true
----
