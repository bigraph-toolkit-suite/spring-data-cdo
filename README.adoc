...
*Current version:* v0.5.0-SNAPSHOT
...
As with all EMF-related programming, the respective `EPackage` must be registered in the global package registry first (see https://download.eclipse.org/modeling/emf/emf/javadoc/2.9.0/[EPackage.Registry]).
...
[source,bash]
----
 $ mvn package deploy -DskipTests -Pdeploy -Dusername=USERNAME -Dpassword=PASSWORD-HASHED
----
